...
data <- data_model
p_fthb <- data %>% filter(flag_fthb != 9) %>% dplyr::count(flag_fthb,default) %>% mutate(n_pct = n / sum(n) * 100) %>% ggplot(aes(flag_fthb, n_pct)) + geom_col(aes(fill=default), show.legend = F) + labs(title="New Homeowner", x = NULL, y = '%') + th+ scale_fill_manual(values = c(basem3, basem4))
p_unit <- data %>% filter(cnt_units != 99) %>% dplyr::count(cnt_units,default) %>% mutate(n_pct = n / sum(n) * 100) %>% ggplot(aes(cnt_units, n_pct)) + geom_col(aes(fill=default), show.legend = F) + labs(title="Number of Units", x = NULL, y = NULL) + th+ scale_fill_manual(values = c(basem3, basem4))
p_occ  <- data %>% dplyr::count(occpy_sts,default) %>%mutate(n_pct = n / sum(n) * 100) %>% ggplot(aes(occpy_sts, n_pct)) + geom_col(aes(fill=default), show.legend = F) + labs(title="Ocupancy Status", x = NULL, y = NULL) + th+ scale_fill_manual(values = c(basem3, basem4))
p_chan <- data %>% dplyr::count(channel,default) %>% mutate(n_pct = n / sum(n) * 100) %>% ggplot(aes(channel, n_pct)) + geom_col(aes(fill=default), show.legend = F)+ labs(title="Channel", x = NULL, y = NULL) + th+ scale_fill_manual(values = c(basem3, basem4))
p_prod <- data %>% dplyr::count(prod_type,default) %>% mutate(n_pct = n / sum(n) * 100) %>% ggplot(aes(prod_type, n_pct)) + geom_col(aes(fill=default), show.legend = F)+ labs(title="Loan Type", x = NULL, y = NULL) + th+ scale_fill_manual(values = c(basem3, basem4)) 
p_prop <- data %>%  dplyr::count(prop_type,default) %>% mutate(n_pct = n / sum(n) * 100) %>% drop_na() %>% ggplot(aes(prop_type, n_pct)) + geom_col(aes(fill=default), show.legend = F)+ labs(title="Property Type", x = NULL, y = '%') + th+ scale_fill_manual(values = c(basem3, basem4))
p_purp <- data %>% dplyr::count(loan_purpose,default) %>% mutate(n_pct = n / sum(n) * 100) %>% ggplot(aes(loan_purpose, n_pct)) + geom_col(aes(fill=default), show.legend = F)+ labs(title="Purpose of Loan", x = NULL, y = NULL) + th+ scale_fill_manual(values = c(basem3, basem4))
p_term <- data %>% dplyr::count(orig_loan_term,default) %>% filter(n>200) %>% mutate(n_pct = n / sum(n) * 100) %>% ggplot(aes(as.factor(orig_loan_term), n_pct)) + geom_col(aes(fill=default))+ labs(title="Number of Terms", x = NULL, y = NULL, fill="default") + 
  th + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_fill_manual(values = c(basem3, basem4))
p_borr <- data %>% filter(cnt_borr != 99) %>% dplyr::count(cnt_borr,default) %>% mutate(n_pct = n / sum(n) * 100) %>% ggplot(aes(cnt_borr, n_pct)) + geom_col(aes(fill=default), show.legend = F) + labs(title="Number of Borrowers", x = NULL, y = NULL) + th+ scale_fill_manual(values = c(basem3, basem4))
