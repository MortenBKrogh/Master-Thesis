...
preprocessing_steps <-
  recipe(default ~ ., data = balanced.train) %>%
  ...
  # (3) Last step prep
  prep(data = balanced.train)

x_balanced.train <- bake (preprocessing_steps, new_data = balanced.train) %>% 
  select (-default)
x_balanced.test <- bake (preprocessing_steps, new_data = balanced.test) %>% 
  select (-default)

y_balanced.train <- ifelse (pull (balanced.train, default) == TRUE, 1, 0)
y_balanced.test  <- ifelse (pull (balanced.test, default)  == TRUE, 1, 0)
...
ANN_v1 <- keras_model_sequential() %>%
  ...
  layer_dense (units              = 128, #=> Num Of Nodes
               ...
               activation         = "relu",    
               input_shape        = ncol(x_balanced.train)) %>% 
  ...
  compile (optimizer = 'adam', #=> Most Popular for Optimization Algo.
           ...
           metrics   = c('accuracy') ) #=> balanced.train/balanced.test Evaluation
...
system.time ( 
  history <- fit (
    ...
    epochs           = 35,     #=> Control balanced.training cycles
...
system.time (
  explanation <- lime::explain (
    x_balanced.test[1:10, ], # Just to show first 10 cases
    explainer    = explainer, 
...
plot_features (explanation) +
  labs (title = "LIME: Feature Importance Visualization",
        subtitle = "Hold Out (balanced.test) Set, First 10 Cases Shown")
